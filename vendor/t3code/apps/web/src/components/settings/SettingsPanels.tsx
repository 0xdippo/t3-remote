...
import { useCodexProfilesMutations, useCodexProfilesQuery } from "../../hooks/useCodexProfiles";
import {
...
function CodexProfilesCard() {
  const { data, isLoading } = useCodexProfilesQuery();
  const { addProfile, removeProfile, switchProfile, reauthProfile } = useCodexProfilesMutations();
  ...
  const handleAddProfile = useCallback(() => {
    ...
    if (!trimmed) {
      setFormError("Enter a profile name.");
      return;
    }
    setFormError(null);
    addProfile.mutate({ label: trimmed });
    setNewProfileLabel("");
  }, [addProfile, newProfileLabel]);
  ...
  const switchDisabled = data.switchLocked || switchProfile.isPending;

  return (
    <div className="space-y-3 p-4">
      {data.switchLocked && data.lockInfo ? (
        <Alert variant="warning">
          <AlertTitle>Switching disabled</AlertTitle>
          <AlertDescription>
            {data.lockInfo.reason}
            {data.lockInfo.runningThreads.length > 0 ? (
              <ul className="mt-2 space-y-1 text-xs">
                {data.lockInfo.runningThreads.map((thread) => (
                  <li key={thread.threadId} className="truncate font-mono text-muted-foreground/90">
                    {thread.title}
                  </li>
                ))}
              </ul>
      ...
      <div className="space-y-2">
        {data.profiles.map((profile) => {
          ...
          return (
            <div
              key={profile.id}
              className="rounded-xl border border-border/60 bg-card/40 p-3 sm:flex sm:items-center sm:justify-between"
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-foreground">{profile.label}</span>
                  {isActive ? (
                    <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300">
                      Active
                    </span>
                  ) : null}
                  {!profile.managed ? (
                    <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] text-muted-foreground/80">
                      External
                    </span>
                  ) : null}
                </div>
                <div className="text-xs text-muted-foreground">
                  Codex home: <code className="break-all">{profile.homePath}</code>
                </div>
                {profile.lastAuthLabel ? (
                  <div className="text-xs text-muted-foreground/80">
                    Auth: {profile.lastAuthLabel}
                    {profile.lastAuthType ? ` (${profile.lastAuthType})` : null}
                  </div>
                ) : (
                  <div className="text-xs text-muted-foreground/60">Auth status unknown.</div>
                )}
              </div>
              <div className="mt-3 flex items-center gap-2 sm:mt-0 sm:justify-end">
                <Button
                  ...
                  onClick={() => switchProfile.mutate({ profileId: profile.id })}
                >
                  {isActive ? "Active" : switchProfile.isPending ? "Switching…" : "Switch"}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  disabled={reauthProfile.isPending}
                  onClick={() => reauthProfile.mutate({ profileId: profile.id })}
                >
                  {reauthProfile.isPending ? "Working…" : "Re-auth"}
                </Button>
                <Button
                  size="sm"
                  variant="ghost"
                  disabled={isActive || removeProfile.isPending}
                  onClick={() => removeProfile.mutate({ profileId: profile.id })}
                >
                  Remove
                </Button>
              </div>
      ...
      <div className="border-t border-border/60 pt-3">
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          >
            {addProfile.isPending ? "Saving…" : "Add profile"}
          </Button>
        </div>
  ...
  return (
    <SettingsPageContainer>
      ...
      <SettingsSection
        ...
        icon={<BotIcon className="size-3 text-muted-foreground" />}
      >
        <CodexProfilesCard />
      </SettingsSection>
